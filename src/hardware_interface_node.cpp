...
// TODO:
// --> strutture per dati di sterzo
// --> logo maila la lumaca 
// --> controllare gli altri servomotori
// --> controllare delay attuazione
// --> iniziare a lavorare da prima
// --> rallentare con calma
// --> creare git
...
#define STEER_SX_CH 2
#define STEER_DX_CH 1
#define MOTOR_CH 0
#define PIN_BASE 300
#define MAX_PWM 4096
#define HERTZ 50
...
void load_params(const ros::NodeHandle& nh){
    ...
    max_steering = getParam<double>(nh, "max_steering");    
    steer_range  = getParam<double>(nh, "steer_pwm/range");
    steer_center = getParam<double>(nh, "steer_pwm/center");
    steer_dead_range = getParam<double>(nh, "steer_pwm/dead_range");
    
    max_speed    = getParam<double>(nh, "max_speed");
    motor_range  = getParam<double>(nh, "motor_pwm/range");
    motor_center = getParam<double>(nh, "motor_pwm/center");
    motor_dead_range = getParam<double>(nh, "motor_pwm/dead_range");
 ...
 */
int calcPWM(const float duty_cycle)
{	
	return (int)(MAX_PWM * duty_cycle + 0.5f);
}

void setSteeringAngle(float value){		
	value = trimValue(value/max_steering);
	float duty_cycle = map(value, steer_center - steer_range, 
							steer_center + steer_range, steer_dead_range);

	int tick = calcPWM(duty_cycle);

	pwmWrite(PIN_BASE + STEER_DX_CH, tick);
	pwmWrite(PIN_BASE + STEER_SX_CH, tick);
	return;
}

void setThrottle(float value){
	value = trimValue(value/max_speed);
	float duty_cycle = map(value, motor_center - motor_range, 
							motor_center + motor_range, motor_dead_range);
...
void timerCallback(const ros::TimerEvent& event){  
  ...
  if(!freq_drive_type_ok){
    neutralControl();
    float dt = static_cast<float>((event.current_real  - event.last_real).toSec());
...
bool setUp(){
	...
	fd0 = pca9685Setup(PIN_BASE , PCA9685_ADDRESS, HERTZ);
	if (fd0 < 0)
	{
